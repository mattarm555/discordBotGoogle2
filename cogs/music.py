...
@app_commands.describe(name="Name for the playlist", link="YouTube or SoundCloud playlist link")
...
@app_commands.describe(name="The name of the saved playlist to play")
...
async def list_playlists(self, interaction: Interaction):
    guild_id = str(interaction.guild.id)
    playlists = load_playlists().get(guild_id, {})
    if not playlists:
...
@app_commands.describe(name="The name of the playlist to remove")
