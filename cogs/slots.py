...
class Slots(commands.Cog):
    ...
    @app_commands.command(name="slotsim", description="Simulate slot spins to estimate RTP (no balance impact)")
    @app_commands.describe(spins="Number of simulated spins (10-100000)", wager="Total bet per spin (1-2000)", lines="Lines per spin (1-5)")
    async def slotsim(self, interaction: Interaction,
                      spins: app_commands.Range[int, 10, 100000],
                      wager: app_commands.Range[int, 1, 2000] = 100,
                      lines: app_commands.Range[int, 1, 5] = 5):
        # For heavy simulations defer the response
        await interaction.response.defer(thinking=True, ephemeral=True)
        if wager < lines:
            await interaction.followup.send("❌ Wager must be at least the number of lines (min 1 coin per line).", ephemeral=True)
            return
        line_bet = wager // lines
        if line_bet == 0:
            await interaction.followup.send("❌ Wager too small for selected lines.", ephemeral=True)
            return
        total_bet_sum = 0
        total_win_sum = 0
        hits = 0
        two_match_hits = 0
        jackpot_hits = { '7️⃣':0, '💎':0 }
        ...
        for _ in range(spins):
            ...
            if win > 0:
                hits += 1
            # classify wins (independent of exact multiplier text so tuning doesn't break stats)
            # Count two-match occurrences by scanning notes lines for the literal tag
            for n in notes:
                if '2-match' in n:
                    two_match_hits += 1
            # Jackpot (any line containing exact triple of symbol)
            for pattern in PAYLINES[lines]:
                line_syms = [window[row][col] for col, row in enumerate(pattern)]
                if line_syms[0] == line_syms[1] == line_syms[2]:
                    if line_syms[0] == SEVEN:
                        jackpot_hits['7️⃣'] += 1
                    elif line_syms[0] == DIAM:
                        jackpot_hits['💎'] += 1
        net = total_win_sum - total_bet_sum
        rtp = (total_win_sum / total_bet_sum * 100) if total_bet_sum else 0
        hit_rate = hits / spins * 100
        two_match_rate = two_match_hits / spins * 100
        embed = discord.Embed(title="🎰 Slot Simulation",
                              description=(
                                  f"Spins: {spins}\n"
                                  f"Lines: {lines} (line bet {line_bet})\n"
                                  f"Total Bet: {total_bet_sum}\n"
                                  f"Total Won: {total_win_sum}\n"
                                  f"Net: {'+' if net>0 else ''}{net}\n"
                                  f"RTP: {rtp:.2f}%\n"
                                  f"Hit Rate: {hit_rate:.2f}% (any win)\n"
                                  f"2-Match Rate: {two_match_rate:.2f}%\n"
                                  f"Jackpots 7️⃣: {jackpot_hits['7️⃣']} | 💎: {jackpot_hits['💎']}"
                              ),
